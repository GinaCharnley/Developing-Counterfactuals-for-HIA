...
3.  Next, we look a the **incorporation of greenhouse gas concentrations** as the forcing proxy, rather than using GMTA. Allowing attribution to GHGs, rather than to climate change and global warming more generally.

4.  We move on to c**limate event counterfactuals**, where we explore:

    1.  **World without the event** using Fouriers regression and linear interpolation

    2.  **World with different event** using qualitative storylines, empirical relationships in atmospheric thermodynamics and stochastic weather generators.

5.  Finally, we provide some code for generating **confidence intervals** for your counterfactual climate variables via boostrapping.
...
-   **Generalised Additive Models (GAMs)** via the `mgcv` package
...
concentration <- read_delim("04_ghg_concentrations_processing/concentration_data.csv",delim = ";", escape_double = FALSE, trim_ws = TRUE)

# === Step 2: Merge GHG to climate data ===
climate_merged <- left_join(climate_merged, concentration)
climate_merged2 <- climate_merged %>% select(lat, lon, year, month, temperature, conc_co2)
...
# Plot
ggplot(annual_loc2, aes(x = as.numeric(year))) +
  geom_line(aes(y = temp), color = "#FF6F61", linewidth = 1, alpha = .6) +
  geom_line(aes(y = concen_cf), color = "dodgerblue", linewidth = 1, alpha = .6) +
  theme_minimal() + 
  labs(title = "Annual Observed (red) vs Counterfactual Temperature using Carbon Dioxide Concentration Forcing (blue)",
       y = "Temperature (°C)", x = "Year") +  
  theme(text = element_text(face = "bold"))  
```
...
So far the methods outlined have only covered climate trend counterfactuals, but **event-based attribution for HIA** is also a growing field. It is particularly important to explore simple and novel methods of developing event counterfactuals for impact studies, as these may not be available in "off-the-shelf" datasets.

While trend-based analysis require detrending or ISIMIP scenario data for the counterfactual, event-based attribution is different, and there are a number of methods to generate your climate variable counterfactual. The main difference lies in how you **generate the “differing event” scenario** and **how much physical realism you impose**.
...
It should be noted here that we are not attributing the extreme event to climate change, but rather the extreme event (or components of the extreme event, which may have been altered by anthropogenic climate change) to the health outcome.

### “World without the event” Counterfactual

The most simple method to assess the impacts of an extreme event on a health outcome is to simply **remove the outlier** which corresponds to the extreme event e.g., the heatwave, storm etc.

Two methods are used here to remove the events, the first being **Fourier regression**, which fits sine and cosine waves to capture smooth, repeated seasonal cycles without short-term anomalies (great for period signals, but not for abrupt seasonal transitions) and the second using a **simple time interpolation** (simple linear interpolation which is better for short-term events). You could also use some of the other methods explored here including loess splines, general additive models and decomposition methods.
...
df <- read_csv("02_synthetic_climate_data_generation/synthetic_heatwaves.csv")
...
ggplot(plot_data, aes(x = date)) +
  ...
  geom_line(aes(y = counterfactual_seasonal, color = "Counterfactual (Fouriers)"), size = 0.8, linetype = "dashed", alpha = 0.8) +
  geom_line(aes(y = counterfactual_interp, color = "Counterfactual (Interpolated)"), size = 0.8, linetype = "dashed", alpha = 0.8) +
  ...
  scale_color_manual(values = c(
    "Original" = "black",
    "Counterfactual (Fouriers)" = "purple",
...
Empirical relationships are global estimates and how weather translates regionally can be very different, therefore this is **better for larger spatial scales**. Some important and useful ones for generating a counterfactual climate variable include:
...
Example - creating a **2 degrees Celsius counterfactual** for the same temperature data and scaling for precipitation, water vapour and CAPE.

```{r formulas}
# === Step 1: Define your warming ===
delta_T <- 2  # °C

# === Step 2: Build your counterfactual based on the scaling above ===
df <- df %>%
  dplyr::mutate(
...
```{r plot 6}
# Slice one year for visualisation
sample_year <- sample(unique(year(df$date)), 1)

plot_data <- df %>% 
  filter(year(date) == sample_year,
         month(date) %in% 6:8)

# Plot precipitation and its counterfactual using the scaling 
ggplot(plot_data, aes(x = date)) +
  geom_line(aes(y = P_mean,    colour = "Observed"),      size = 0.8, alpha = 0.7) +
  geom_line(aes(y = P_mean_cf, colour = "Counterfactual"), size = 0.8, linetype = "dashed", alpha = 0.9) +
  geom_rect(
    data = plot_data %>% filter(is_heatwave),
    aes(xmin = date - 0.5, xmax = date + 0.5,
        ymin = -Inf, ymax = Inf),
    inherit.aes = FALSE,
    fill = "#FF6F61", alpha = 0.15
  ) +
  ...
  scale_x_date(date_labels = "%b %d", date_breaks = "1 week") +
  labs(
    title  = paste("Precipitation and Counterfactual\nSummer", sample_year),
    x      = "Date",
    y      = "Precipitation (mm)",
    colour = "Series"
  ) +
...
SWGs are useful because they can preserve long-term patterns in the weather, but **add the much needed stochastic variation that is lost in counterfactuals climate varaibles**, particularly detrending exercises. They are also **highly flexibility** and can be tweaked to whatever you need, creating realistic alternative versions of an event to explore your health outcome.

Example - using the synthetic heatwave temperature data, a generic ensemble is generated of plausible future daily temperature series (from the end of the dataset, 2020 to 2029) and heatwaves for many years using **Fourier harmonics** (used above) and **single AR(1) and Guassian noise** to incorporate the stochasticity.

```{r swg}
# === Step 1: Set the parameters ===
input_csv <- "02_synthetic_climate_data_generation/synthetic_heatwaves.csv"
