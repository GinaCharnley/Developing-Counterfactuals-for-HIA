...
If you want to look at the impact of **long-term climatic changes** on a health outcome (e.g., a communicable or non-communicable diseases) you would use **trend-to-trend** analysis. Applying your known health-climate relationship over a long period of **historical** and **counterfactual climate data**, and estimating how the outcome differs in the two worlds.

Limitations of this approach include the assumption of a *static exposure-response relationship*, and significant *smoothing of climatic variability* in the counterfactual.

You can also explore the impact of a specific or multiple **extreme weather events** on a known health outcome that occurred during or after the event. Here, you would use **risk-based** (quantifies how climate change has altered the risk of a health outcome by directly linking it to a specific climate event) or **event-to-event** methods, which explores the way that anthropogenic influences altered the event and therefore its related health outcome. These event-based methods commonly use **story-line or descriptive frameworks** to explore the event in differing scenarios.

Limitations of event-to-event analysis are the *exclusion of social determinants* of the health outcome, which in complex settings such as extreme events, are often highly related to the health outcome. Additionally, at extremely high pressures, *storm systems enter a dynamically unstable regime*, where even small variations in initial conditions can lead to drastically different outcomes—making alternative scenarios inherently unreliable.

## Trend Counterfactuals via Detrending

There are many methods of detrending, but they are all *based on the assumption that the most recent warming is due to anthropogenic climate change*. Detrending is particularly useful when working with observational datasets, rather than model simulations. Methods can be **linear**, which are simple and interpretable, or **non-linear**, which are more flexible but easily overfit. Below in the table shows a list of common detrending methods, however, this is not an exhaustive list, and the field is rapidly evolving.
...
Before getting starting with creating your own counterfactual using detrending, I recommend reading "ATTRICI v1.1 - counterfactual climate for impact attribution" (Mengel *et al*., 2021), there is also a summary available at: <https://www.isimip.org/documents/481/ATTRICI_-_counterfactual_climate_for_impact_attribution.pdf>. This is a seminal paper for detrending to create climate counterfactuals and gives a great overview of the topic.

## Linear Residual-based Detrending (ATTRICI-style)

In order to develop a counterfactual using a model residuals-based detrending you need two datasets:

1.  A time-series of the **observed climate variable**, for the location and the scale that you want
2.  Long-term global mean temperature anomalies (**GMTA**), which are a time-series of temperature anomalies relative to a pre-industrial baseline (1850-1900)

Here, we make use of the [Berkeley Earth](https://berkeleyearth.org/data/) GMTA dataset, opting for the annual data at the pre-industrial period of 1850-1900.

Now you have your data, you want to **regress out the anthropogenic trend** (as approximated by the GMTA) from your observed data. It should be noted that despite using this methodology for trend counterfactuals, you could also use it for a long-term time-series of event data, such as heatwaves.

At each grid cell or location, regress the local variable (e.g., temperature) on the GMTA, then subtract the fitted trend, which leaves you with a counterfactual that preserves local variability (e.g., El Niño), but removes global climate forcing.

### Data

To complete this tutorial a number of datasets have been either extracted or synthesised. To ease computational time and resources, the temporal and spatial limits of the data are Delhi, India from 1920-2024. The raw Berkeley Earth GMTA data have been downloaded, these data are monthly to a 1x1 degree grid cell. The data have been converted to annual, using a weighted mean to account for different month length, subset to the Delhi region and saved in a .csv format. The data and processing script are saved in the file "gmta".

The climate data are synthetically processed to match the spatial and temporal limits and resolution of the GMTA data. Generated are datasets for temperature, precipitation, humidity and heatwaves (these were not assigned to a specific location, and covered 2010-2019). The data and processing script are saved in the file "climate".
...
```{r load and merge}
# Load the datasets 
temp <- read.csv("climate/temperature.csv")
gmta <- read.csv("gmta/anomalies.csv")
...
In ATTRICI, you’re trying to estimate **counterfactual trajectories** for time series under hypothetical interventions. The GMTAs are essentially the **time-varying effect estimates** that form the building blocks for these counterfactuals.

However, raw GMTAs are often **noisy** because they are estimated from observational data with temporal fluctuations. If you use them directly, your counterfactual will inherit this noise. Smoothing them with SSA (more information on SSA later) makes the counterfactual trajectory **less noisy, more realistic, and preserves meaningful trends and cycles** rather than short-term fluctuations.
...
$$
\text{LocalTemp}_{t,m} = \alpha_m + \beta_m \cdot \text{GMTA}_{t} + \varepsilon_{t,m}
$$
...
climate_merged <- climate_merged %>%
  group_by(month) %>%
  mutate(
    trend = predict(lm(temperature ~ smoothed_anomaly, data = pick(everything()))),
    temp_cf = temperature - trend + mean(temperature, na.rm = TRUE)
  ) %>%
  ungroup()
```
...
To visualise the differences in the **observed v the counterfactual** below is a plot of the annual average temperature values for the full region.
...
# Plot
ggplot(annual_loc, aes(x = as.numeric(year))) +
  geom_line(aes(y = temp), color = "#FF6F61", linewidth = 1, alpha = .6) +
  geom_line(aes(y = temp_cf), color = "dodgerblue", linewidth = 1, alpha = .6) +
  theme_minimal() + 
  labs(title = "Annual Observed (red) vs Counterfactual Temperature (blue)",
...
-   $Y_{\text{hist}}$ = predicted health outcome using *observed* climate

-   $Y_{\text{cf}}$ = predicted outcome using *counterfactual* climate
...
## Non-Linear Residual-based Detrending
...
Loess or GAM lets the relationship between global anomaly and the local climate variable **change shape** — it’s not forced to be a straight line. As these methods are very similar, the sole difference being the replacement of the linear term with a non-linear term, the data input is the same, along with the communication of the results.

## Counterfactuals Without GMTAs

You can generate counterfactuals without using your GMTAs (see Table above in "Trend Counterfactuals via Detrending" section). Using a method without GMTAs may be particularly desirable if you are not interested in temperature, as there are no anomaly data for other variables such as precipitation or humidity, and therefore you have to use the temperature anomalies as a proxy (more about this in the next section), which has its limitations.

Here, I provide details for two methods of creating a counterfactual using **data-driven decomposition methods**, which separate long-term trends from variability, allowing you to reconstruct a counterfactual by removing the trend-like component.

### 1. Singular Spectrum Analysis (SSA)

SSA is already explored above, where we use it to smooth the GMTAs, before fitting them in the linear model. SSA is best for **smooth trends** and decomposes into trends + oscillations using **eigen-decomposition**.
...
EMD is better for **non-linear and adaptive** data and extracts **intrinsic mode functions (IMFs)** + the residual (trend).
...
You therefore want to reconstruct your counterfactual by **removing the low-frequency trend component**.
...
Additionally, any of the **detrending approaches** outlined above could be **used on your extreme events** to develop a pre-industrial event counterfactual.
...
Note here that both methods are relatively **data hungry** and tends to work better on long time-series (the data are chosen here to speed computational time in the example). To maximise use and time, I would opt for **SSA** when using a **long time-series and wide geographic area** (particularly annual data which has a clearer trend with the seasonality removed, as its better for linear trends), and **EMD** for a **small geographic area** and a **shorter time-series** (although longer is going to make it perform better).

## Non-Temperature Counterfactuals

Unlike temperature, global mean anomalies for commonly used climate variables such as precipitation and humidity are less commonly used or straightforward, because:

-   **Precipitation** is spatially highly variable and influenced by many local/regional factors and is not as well captured in the models compared to temperature.

-   **Humidity** (e.g., relative humidity or specific humidity) also varies regionally and seasonally, and global means are less commonly reported as simple anomalies like temperature.

A lack of global mean anomalies from pre-industrial times is a significant reason why most of HIA research has focused on heat and temperature. But you can create a counterfactual for variables like precipitation and humidity, which are often highly influential to health outcomes, in the absence of complex climate models using the same methods explored above for temperature.

Due to more complex and spatially heterogeneous patterns in precipitation and humidity data, an important first step would be to plot a time-series of the data for the location of interest. If there is no long-term trend, then using a decomposition method would not be practical, as there would be no trend to decompose.

Regardless of if there is/is not a trend, you can also use the same residual-based approach used for temperature, and apply this to precipitation or humidity using the linear or non-linear methods explored above. Since precipitation and humidity are physically linked to temperature, you can use the smoothed GMTA or the local temperature forced response as a proxy forcing variable for regression. This assumes the main anthropogenic forcing signal is captured by temperature change (which is a reasonable first approximation).

### R Implementation

#### Temperature change as a forcing proxy

A simple method to start is by applying the linear model residual-based detrending to precipitation and humidity using smoothed GMTA as the forcing predictor. The data used here are the synthesised precipitation and humidity data for the same geographical and spatial scales and resolutions as described above in the "Data" section.

```{r non-temp}
# === Step 1: Load and merge the data ===
precip <- read.csv("climate/precipitation.csv")
humid <- read.csv("climate/humidity.csv")
gmta <- read.csv("gmta/anomalies.csv")

climate_merged2 <- left_join(precip, humid)
climate_merged2 <- left_join(climate_merged2, gmta)
climate_merged2 <- na.omit(climate_merged2)

# === Step 2: Smooth the GMTA ===
ssa_gmta <- ssa(climate_merged2$temperature_anomaly_annual, L = 10)
gmta_trend <- reconstruct(ssa_gmta, groups = list(1:3))$F1
climate_merged2$smoothed_anomaly <- gmta_trend

# === Step 3: Fit the model and remove the trend ===
climate_merged2 <- climate_merged2 %>%
  group_by(month) %>%
  mutate(
    trend_pre = predict(lm(precipitation_mm ~ smoothed_anomaly, data = pick(everything()))),
    pre_cf = precipitation_mm - trend_pre + mean(precipitation_mm, na.rm = TRUE)
  ) %>% 
  mutate(
    trend_hum = predict(lm(humidity_percent ~ smoothed_anomaly, data = pick(everything()))),
    hum_cf = humidity_percent - trend_hum + mean(precipitation_mm, na.rm = TRUE)
  ) %>%
  ungroup()
```

Plot the observed v counterfactual for precipitation and humidity.

```{r plot 3}
# Group by year and compute annual mean
annual_loc2 <- climate_merged2 %>%
  group_by(year) %>%
  summarise(
    precip = mean(precipitation_mm, na.rm = TRUE),
    precip_cf = mean(pre_cf, na.rm = TRUE),
    humidity = mean(humidity_percent, na.rm = TRUE),
    humid_cf = mean(hum_cf, na.rm = TRUE)
  )

# Plot
p1 <- ggplot(annual_loc2, aes(x = as.numeric(year))) +
  geom_line(aes(y = precip), color = "dodgerblue", size = .6) +
  geom_line(aes(y = precip_cf), color = "#93cccc", size = 1, alpha = 0.5) +
  theme_minimal() + 
  labs(title = "Annual Observed vs Counterfactual (faded shade)",
       y = "Precipitation (m)", x = "Year") +  
  theme(text = element_text(face = "bold"))  

p2 <- ggplot(annual_loc2, aes(x = as.numeric(year))) +
...
Instead of using GMTA as your forcing variable, you may instead want to account for specific drivers like **CO₂ emissions** and **land-use change**. You can continue to use the ATTRICI-style detrending to accomplish this, but instead switch out your GMTA for a specific emitter or greenhouse gas concentration. You should only **include one driver at once**, (or GMTA), as these will all be **highly correlated** and make you $\beta$ values hard to interpret.
...
The main advantage to doing this, is so that you can **attribute your health outcome to greenhouse gas emissions** or a **specific driver/emitter**, whereas, with solely GMTA, you can only attribute more generally to climate change/global warming.
...
-   **Driver time-series** (matched in time to your climate data) - Greenhouse gas concentrations and/or *emissions* e.g., CO₂, CH₄, N₂O, O₃ via burning of fossil fuels or land use change.

There is a more in-depth look into potential emitter concentrations and emissions data in a separate file labelled *"drivers.R"*. Here, the chosen data is also processed and cleaned. Some emissions data are country, industry or company dis-aggregated, however, it should be noted that if you intend to attribute a health outcome to any of these entities, it remains difficult because atmospheric mixing distributes pollutants globally.

It is worth nothing that here the incorporation of land use change is solely related to emissions created by changing land use, it would **not give a land use counterfactual** e.g., land cover in pre-industrial times. For this you would need to [HYDE](https://www.pbl.nl/en/hyde-history-database-of-the-global-environment) dataset (see link for more information).

### R Implementation

The example below includes the synthetic temperature data, with the exploration of three separate forcing components in the models, concentrations of atmospheric CO₂, all global fossil fuel emissions and all global land use emissions.
...
Here, I am using artificial temperature and heatwave data which is simulated in the*"simulated_data.R"* script in the "climate" folder, see R script for more details. The data are not location specific, however, they do include general global trends in temperatures including warming, seasonality and noise. A heatwave is set at a thresholds of more than or equal to 3 days above the 90th percentile.
